(() => {
  ...
  function renderVehicles() {
    ...
    updateFilmCertificateContext()
  }
  function updateFilmCertificateContext() {
    const target = $("film-certificate-context")
    if (!state.member) { target.textContent = "请先在上方查询会员并选择服务车辆。"; return }
    const vehicle = state.vehicles.find(item => item.id === $("film-vehicle").value && item.status === 1)
    if (!vehicle) { target.textContent = `当前客户：${state.member.name || "会员"}（${state.member.mobile}）· 请先新增并选择服务车辆。`; return }
    target.textContent = `客户姓名：${state.member.name || "会员"} · 联系电话：${state.member.mobile} · 汽车品牌：${vehicle.brand || "未填写"} · 车牌号码：${vehicle.plateNumber || "未填写"}`
  }
  ...
  $("film-vehicle").addEventListener("change", updateFilmCertificateContext)
  ...
  $("add-film").addEventListener("click", () => submitting($("add-film"), async () => { try { if (!state.member) throw new Error("请先查询会员"); if (!$("film-vehicle").value) throw new Error("请先新增并选择车辆"); const warrantyYears = Number($("film-warranty-years").value); if (!Number.isSafeInteger(warrantyYears) || warrantyYears < 0 || warrantyYears > 20) throw new Error("质保年限请填写 0 至 20 的整数"); const constructionPriceFen = fen($("film-price").value); if (!Number.isSafeInteger(constructionPriceFen) || constructionPriceFen < 0) throw new Error("请填写正确的施工价格"); message("照片上传中..."); const images = await uploadImages($("film-images").files); await call("staffAddFilm", { token: state.token, userId: state.member.id, vehicleId: $("film-vehicle").value, serviceDate: $("film-service-date").value, constructionStore: $("film-store").value, constructionPriceFen, filmCategory: $("film-category").value, productType: $("film-product-type").value, filmBrand: $("film-brand").value, filmSeries: $("film-series").value, filmModel: $("film-model").value, rollNumber: $("film-roll-number").value, installPosition: Array.from(document.querySelectorAll("#film-positions input:checked")).map(item => item.value), warrantyMonths: warrantyYears * 12, mileageKm: Number($("film-mileage").value), images, remark: $("film-remark").value }); message("质保档案已保存，顾客端可立即查看"); $("film-images").value = ""; $("film-remark").value = ""; $("image-preview").replaceChildren() } catch (error) { message(error.message, "error") } }))
