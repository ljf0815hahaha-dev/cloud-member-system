...
async function requireStaff(db, token) {
  ...
  if (!session || !Number.isFinite(expireAt) || expireAt <= Date.now() || !["staff", "admin"].includes(session.role)) throw unauthorized()
  ...
  const version = staff.tokenVersion == null ? 1 : Number(staff.tokenVersion)
  if (session.tokenVersion == null ? staff.tokenVersion != null && version !== 1 : Number(session.tokenVersion) !== version) throw unauthorized()
  return staff
}
